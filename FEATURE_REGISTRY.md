...
- First-vision Draft Session notes are additive; do not write contrast/clarity into `vision_versions` until the compose pass is accepted
...
Schema: `activation_kits` (saved presets, one default per user), `activation_kit_runs` (per-asset `asset_status`, both owner-only RLS + realtime). API: `/api/activation-kit/generate`, `/api/activation-kit/board-suggestions`, `/api/activation-kit/runs/[id]`, `/api/activation-kit/kits` (CRUD). Lib: `src/lib/activation-kit/*` (separate namespace from the lead-magnet `src/lib/activation/`). Prompts: `src/lib/viva/prompts/activation-kit-prompts.ts`.
