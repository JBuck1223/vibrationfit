...
const Body = z.object({
  ...
  toneHints: z.array(z.string()).default(["warm","grounded"]),
...
function computeCompletion(record: Record<string, any>) {
  const filled = CATS.filter(c => (record[c] ?? "").trim().length > 0).length;
...
export async function POST(req: NextRequest) {
  ...
  // 2) Call GPT-5 to compose the chunk
