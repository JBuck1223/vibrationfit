...
CREATE OR REPLACE FUNCTION "public"."deduct_tokens_with_fifo"("p_user_id" "uuid", "p_tokens_to_deduct" integer) RETURNS "jsonb"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
...
ALTER FUNCTION "public"."deduct_tokens_with_fifo"("p_user_id" "uuid", "p_tokens_to_deduct" integer) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."deduct_tokens_with_fifo"("p_user_id" "uuid", "p_tokens_to_deduct" integer) IS 'Deducts tokens from oldest grants first (FIFO), skips expired grants';
...
CREATE OR REPLACE FUNCTION "public"."drip_tokens_28day"("p_user_id" "uuid", "p_subscription_id" "uuid", "p_cycle_number" integer) RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
...
COMMENT ON FUNCTION "public"."drip_tokens_28day"("p_user_id" "uuid", "p_subscription_id" "uuid", "p_cycle_number" integer) IS 'Drip 375k tokens per 28-day cycle with 90-day expiry (3-cycle max rollover = 1.125M)';
...
CREATE OR REPLACE FUNCTION "public"."expire_old_token_grants"() RETURNS integer
    LANGUAGE "plpgsql"
    AS $$
DECLARE
...
ALTER FUNCTION "public"."expire_old_token_grants"() OWNER TO "postgres";


COMMENT ON FUNCTION "public"."expire_old_token_grants"() IS 'Background job: marks expired token grants. Returns count of expired grants.';
...
CREATE OR REPLACE FUNCTION "public"."get_user_token_balance"("p_user_id" "uuid") RETURNS TABLE("total_active" integer, "total_expired" integer, "grants_breakdown" "jsonb")
    LANGUAGE "plpgsql" STABLE
    AS $$
BEGIN
  RETURN QUERY
...
COMMENT ON FUNCTION "public"."get_user_token_balance"("p_user_id" "uuid") IS 'Returns active and expired token balances with breakdown by grant type';
...
COMMENT ON FUNCTION "public"."grant_annual_tokens"("p_user_id" "uuid", "p_subscription_id" "uuid") IS 'Grant 5M tokens + 100GB storage for annual subscription (365-day expiry)';



CREATE OR REPLACE FUNCTION "public"."grant_trial_tokens"("p_user_id" "uuid", "p_subscription_id" "uuid", "p_tokens" integer, "p_trial_period_days" integer) RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
...
ALTER FUNCTION "public"."grant_trial_tokens"("p_user_id" "uuid", "p_subscription_id" "uuid", "p_tokens" integer, "p_trial_period_days" integer) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."grant_trial_tokens"("p_user_id" "uuid", "p_subscription_id" "uuid", "p_tokens" integer, "p_trial_period_days" integer) IS 'Grant trial tokens with custom expiration period + 25GB storage';
...
COMMENT ON TABLE "public"."token_balances" IS 'Tracks individual token grants with FIFO expiration. Deductions consume oldest grants first.';



COMMENT ON COLUMN "public"."token_balances"."grant_type" IS 'Type: annual (365d expiry), 28day (90d expiry), trial (custom expiry), purchase (never expires), admin (custom)';



COMMENT ON COLUMN "public"."token_balances"."tokens_remaining" IS 'Decrements as tokens are consumed (FIFO). 0 = fully consumed.';



COMMENT ON COLUMN "public"."token_balances"."expires_at" IS 'NULL = never expires (purchases). Otherwise auto-expire on this date.';
...
CREATE INDEX "idx_token_balances_expiration" ON "public"."token_balances" USING "btree" ("expires_at") WHERE ("tokens_remaining" > 0);



CREATE INDEX "idx_token_balances_user_active" ON "public"."token_balances" USING "btree" ("user_id", "granted_at") WHERE ("tokens_remaining" > 0);
...
ALTER TABLE ONLY "public"."token_balances"
    ADD CONSTRAINT "token_balances_subscription_id_fkey" FOREIGN KEY ("subscription_id") REFERENCES "public"."customer_subscriptions"("id") ON DELETE SET NULL;
...
CREATE POLICY "Service role has full access to token_balances" ON "public"."token_balances" USING (("auth"."role"() = 'service_role'::"text"));
...
CREATE POLICY "Users can view own token balances" ON "public"."token_balances" FOR SELECT USING (("auth"."uid"() = "user_id"));
...
GRANT ALL ON FUNCTION "public"."deduct_tokens_with_fifo"("p_user_id" "uuid", "p_tokens_to_deduct" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."deduct_tokens_with_fifo"("p_user_id" "uuid", "p_tokens_to_deduct" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."deduct_tokens_with_fifo"("p_user_id" "uuid", "p_tokens_to_deduct" integer) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."expire_old_token_grants"() TO "anon";
GRANT ALL ON FUNCTION "public"."expire_old_token_grants"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."expire_old_token_grants"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."grant_trial_tokens"("p_user_id" "uuid", "p_subscription_id" "uuid", "p_tokens" integer, "p_trial_period_days" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."grant_trial_tokens"("p_user_id" "uuid", "p_subscription_id" "uuid", "p_tokens" integer, "p_trial_period_days" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."grant_trial_tokens"("p_user_id" "uuid", "p_subscription_id" "uuid", "p_tokens" integer, "p_trial_period_days" integer) TO "service_role";
...
GRANT SELECT ON TABLE "public"."token_balances" TO "cursor_reader";
