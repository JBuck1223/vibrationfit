...
import {
  Play, CheckCircle, Circle, ArrowRight, Headphones, Moon, Zap,
  Sparkles, Mic, Music, Trash2, BookOpen, Image, PenLine, FileText,
  Volume2, Clock, Plus, Music2, ChevronDown,
} from 'lucide-react'
...
export default function RitualPage() {
  const {
    visionId, vision, visionLoading, audioSets, audioSetsLoading, refreshAudioSets, checklist,
    activePill,
  } = useAudioStudio()
    ...
    <Container size="xl" className="py-6">
      <Stack gap="lg">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <ChecklistItem
            checked={checklist.hasMainVisionAudio}
            label="Main Vision Audio"
            description={checklist.hasMainVisionAudio ? 'Ready to play' : 'Generate your first audio set'}
            href={checklist.hasMainVisionAudio ? undefined : '/audio/create'}
          />
          <ChecklistItem
            checked={checklist.hasFocusStoryWithAudio}
            label="Focus Story"
            description={checklist.hasFocusStoryWithAudio ? 'Story with audio' : 'Create a focus story'}
            href={checklist.hasFocusStoryWithAudio ? undefined : '/story/new'}
          />
          <ChecklistItem
            checked={checklist.hasPersonalRecording}
            label="Personal Recording"
            description={checklist.hasPersonalRecording ? 'Your voice captured' : 'Record in your voice'}
            href={checklist.hasPersonalRecording ? undefined : '/audio/record'}
          />
        </div>
        ...
        {activePill === 'focus-stories' && <section>
          ...
          {storiesLoading ? (
            ...
            <Card variant="glass" className="p-6 text-center">
              ...
              <p className="text-neutral-400 mb-4">
                {stories.length === 0 ? 'No focus stories yet. Create one to immerse in your vision.' : 'No stories match this filter.'}
              </p>
              {stories.length === 0 && (
                  ...
                  <Link key={story.id} href={`/story/${story.id}`} className="block group">
                    <Card variant="outlined" className="p-4 hover:border-neutral-600 transition-all duration-200 hover:-translate-y-0.5">
                      ...
                      <div className="flex items-center gap-3 text-[11px] text-neutral-500">
                        ...
                        <span>{Math.max(1, Math.ceil((story.word_count || 0) / 200))} min read</span>
                      </div>
                    </Card>
                  </Link>
                )
          ...
          {filteredStories.length > 6 && (
            <div className="flex justify-center mt-4">
              <Button variant="ghost" size="sm" asChild>
                <Link href="/story">View all {filteredStories.length} stories <ArrowRight className="w-3.5 h-3.5 ml-1" /></Link>
              </Button>
            </div>
          )}
        </section>}
...
// ─── Checklist Item ───
function ChecklistItem({ checked, label, description, href }: {
  checked: boolean; label: string; description: string; href?: string
}) {
  const content = (
    <div className={`flex items-center gap-3 p-3.5 rounded-xl border transition-all ${
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-sm font-medium ${checked ? 'text-white' : 'text-neutral-400'}`}>{label}</p>
        <p className="text-[11px] text-neutral-500">{description}</p>
      </div>
      {!checked && href && <ArrowRight className="w-4 h-4 text-neutral-600 flex-shrink-0" />}
    </div>
  )
