...
function SmsTemplateDetailContent() {
  ...
  const handleDuplicate = async () => {
    try {
      const response = await fetch(
        `/api/admin/templates/sms/${templateId}/duplicate`,
        { method: 'POST' }
      )
      if (response.ok) {
        const data = await response.json()
            ...
            <Card className="p-4 md:p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-3 text-xs">
                  <span className={`${charCount > 160 ? 'text-yellow-500' : 'text-neutral-500'}`}>
                    {charCount} chars
                  </span>
                  <span className={`${segmentCount > 1 ? 'text-yellow-500' : 'text-neutral-500'}`}>
              ...
              <FormattedTextarea
                ...
                placeholder="Your SMS message body. Use the toolbar to insert merge tags like {{firstName}}."
                rows={6}
              />

              {charCount > 160 && (
                <p className="text-xs text-yellow-500 mt-2">
                  Messages over 160 characters are sent as multiple segments (billed per segment).
                  Variable substitution may also increase the final character count.
                </p>
              )}
