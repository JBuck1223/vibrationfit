...
export default function StoryAudioGeneratePage({
  ...
  async function loadData() {
    ...
    if (storyData.audio_set_id) {
      const { data: track } = await supabase
        .from('audio_tracks')
        .select('audio_url, text_content')
        .eq('audio_set_id', storyData.audio_set_id)
        .eq('status', 'completed')
        .order('created_at', { ascending: false })
        .maybeSingle()

      if (track?.audio_url && storyTextMatchesTrack(storyData.content, track.text_content)) {
  ...
  async function handleGenerate() {
    ...
    try {
      const response = await fetch(`/api/stories/${storyId}/audio/generate`, {
        method: 'POST',
      ...
      const data = await response.json()
      if (data.audioUrl) setExistingAudioUrl(data.audioUrl)
  ...
  async function handlePreviewVoice() {
    ...
    if (isPreviewing) {
      ...
      previewAudioRef.current.addEventListener('timeupdate', () => {
        if (previewAudioRef.current) {
          setPreviewProgress((previewAudioRef.current.currentTime / previewAudioRef.current.duration) * 100)
        }
      })
      ...
      <Stack gap="lg">
        ...
        <Card className="p-4 md:p-6 lg:p-8">
          ...
          <div className="max-w-md mx-auto space-y-4">
            ...
            >
              {isPreviewing ? (
                <><X className="w-4 h-4 mr-2" /> Stop Preview</>
              ) : (
