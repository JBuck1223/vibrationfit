...
import { Stack, Card, Badge } from '@/lib/design-system/components'
import {
  GraduationCap,
  BookOpen,
  Heart,
...
function CategoryUnitCard({ unit }: { unit: typeof LIFE_CATEGORY_CURRICULUM[0] }) {
...
/**
 * The original full-year curriculum brief (pre-Life Explorer design document).
 * Preserved as reference material inside the admin system inventory.
 */
export function CurriculumBrief() {
  return (
    <Stack gap="lg">
        {/* Philosophy Overview */}
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
            {LIFE_CATEGORY_CURRICULUM.map((unit) => (
              <CategoryUnitCard key={unit.key} unit={unit} />
            ))}
          </div>
        </ExpandableCard>
