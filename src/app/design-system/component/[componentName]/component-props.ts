...
export const COMPONENT_PROPS: Record<string, PropDefinition[]> = {
  ...
  'recording-textarea': [
    { name: 'value', type: 'string', required: true, description: 'Current text value' },
    { name: 'onChange', type: '(value: string) => void', required: true, description: 'Callback when text changes' },
    { name: 'onRecordingSaved', type: '(url: string, transcript: string, type: "audio" | "video", updatedText: string) => Promise<void>', required: false, description: 'Callback when recording is saved (with S3 URL, transcript, recording type, and updated text value)' },
    { name: 'placeholder', type: 'string', required: false, description: 'Placeholder text' },
    { name: 'rows', type: 'number', required: false, defaultValue: '4', description: 'Number of visible text lines' },
    { name: 'label', type: 'string', required: false, description: 'Label text above textarea' },
    { name: 'allowVideo', type: 'boolean', required: false, defaultValue: 'false', description: 'Show video recording button in addition to audio' },
    { name: 'disabled', type: 'boolean', required: false, defaultValue: 'false', description: 'Disable the textarea and recording buttons' },
    { name: 'className', type: 'string', required: false, description: 'Additional CSS classes' },
    { name: 'storageFolder', type: '"journal" | "visionBoard" | "lifeVision" | "alignmentPlan" | "profile" | "customTracks"', required: false, defaultValue: '"journal"', description: 'S3 folder for recording storage' },
    { name: 'category', type: 'string', required: false, description: 'Category for IndexedDB persistence (e.g., "fun", "health", "journal")' },
    { name: 'instanceId', type: 'string', required: false, description: 'Unique identifier for this field (prevents recordings from bleeding across fields)' },
    { name: 'onUploadProgress', type: '(progress: number, status: string, fileName: string, fileSize: number) => void', required: false, description: 'Callback for upload progress tracking' },
    { name: 'recordingPurpose', type: '"quick" | "transcriptOnly" | "withFile" | "audioOnly"', required: false, defaultValue: '"transcriptOnly"', description: 'Recording behavior: quick (no S3, instant transcript), transcriptOnly (S3 backup, delete if discarded), withFile (S3 always kept), audioOnly (S3 storage, no transcription)' },
  ],

  'media-recorder': [
    { name: 'mode', type: '"audio" | "video"', required: false, defaultValue: '"audio"', description: 'Recording mode - audio or video' },
    ...
    { name: 'category', type: 'string', required: false, defaultValue: '"general"', description: 'Category for IndexedDB persistence (e.g., "fun", "health", "journal")' },
    { name: 'instanceId', type: 'string', required: false, description: 'Unique identifier for this recorder instance (prevents data bleeding)' },
    { name: 'storageFolder', type: 'keyof typeof USER_FOLDERS', required: false, defaultValue: '"journalAudioRecordings"', description: 'S3 folder for uploads (e.g., "journalAudioRecordings", "lifeVisionVideoRecordings")' },
    { name: 'recordingPurpose', type: '"quick" | "transcriptOnly" | "withFile" | "audioOnly"', required: false, defaultValue: '"withFile"', description: 'Recording modes: quick (no S3, instant), transcriptOnly (S3 backup, delete if discarded), withFile (S3 always kept), audioOnly (S3 storage, no transcription)' },
    { name: 'enableEditor', type: 'boolean', required: false, defaultValue: 'true', description: 'Enable audio waveform editor for trimming/cutting' },
    { name: 'initialFacingMode', type: '"user" | "environment"', required: false, defaultValue: '"user"', description: 'Initial camera direction (front/back) for video mode' },
    { name: 'allowCameraSwitch', type: 'boolean', required: false, defaultValue: 'true', description: 'Show camera switch button in video mode' },
    { name: 'fullscreenVideo', type: 'boolean', required: false, defaultValue: 'true', description: 'Enable fullscreen video preview during recording' },
  ],
