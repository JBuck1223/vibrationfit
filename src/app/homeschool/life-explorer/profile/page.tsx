  ...
  return (
    <Container size="lg" className="py-10 md:py-14">
      <Stack gap="lg">
        ...
        {/* Life Category depth */}
        <section>
          <h3 className="text-lg font-semibold text-white mb-1">The 12 Facets of Life</h3>
          <p className="text-sm text-neutral-500 mb-3">
            Depth across life over time — not a forced schedule.
          </p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            {categories.map((c) => {
              const lessons = c.expeditions.reduce((s, e) => s + e.lessons_completed, 0)
              return (
                <div key={c.key} className="rounded-xl border border-[#222] bg-[#111] px-3 py-2.5">
                  <p className="text-sm text-white">{c.label}</p>
                  <p className="text-xs mt-0.5 text-neutral-500">
                    {lessons > 0 ? (
                      <span className="text-[#00FFFF]">{lessons} lessons deep</span>
                    ) : (
                      'unexplored'
                    )}
                  </p>
