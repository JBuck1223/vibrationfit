...
export default function ActivityCalendarPage() {
  ...
  const [editing, setEditing] = useState<LeActivityLog | 'new' | null>(null)
                        ...
                        m.media_type === 'video' ? (
                          ...
                          <a key={m.id} href={m.url} target="_blank" rel="noreferrer">
                            ...
                            <img
                              ...
                              className="h-32 w-32 rounded-lg object-cover border border-[#222]"
                            />
                          </a>
                        )
                      )}
  ...
  async function handleFiles(files: File[]) {
    ...
    try {
      for (const file of files) {
        setUploadProgress(0)
        const result = await uploadUserFile('lifeExplorer', file, userId || undefined, (p) =>
          setUploadProgress(p)
        )
