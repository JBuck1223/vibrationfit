...
export async function POST(request: NextRequest) {
  try {
    ...
    const s3Key = `uploads/${userId}/${folder}/${timestamp}-${randomStr}-${sanitizedName}`
    ...
    if (needsVideoProcessing) {
     ...
     if (isAudioFile) {
       ...
       const finalS3Key = `uploads/${userId}/${folder}/${timestamp}-${randomStr}-${finalFilename}`
    ...
    Settings: {
      ...
      OutputGroups: [{
        ...
        OutputGroupSettings: {
          ...
          FileGroupSettings: {
            Destination: `s3://${BUCKET_NAME}/uploads/${userId}/${folder}/processed/`
          }
        },
        Outputs: [{
