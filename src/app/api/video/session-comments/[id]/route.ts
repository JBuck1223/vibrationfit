...
/**
 * PATCH /api/video/session-comments/[id]
 * Edit a session comment (owner only)
 */
export async function PATCH(
  ...
  try {
    ...
    if (updateError) {
      console.error('Error updating session comment:', updateError)
    ...
    console.error('SESSION COMMENT PATCH ERROR:', error)
    return NextResponse.json({ error: error.message || 'Failed to update comment' }, { status: 500 })
  }
}

/**
 * DELETE /api/video/session-comments/[id]
 * Soft delete a session comment (owner or admin)
 */
export async function DELETE(
  ...
  try {
    ...
    if (deleteError) {
      console.error('Error deleting session comment:', deleteError)
      return NextResponse.json({ error: 'Failed to delete comment' }, { status: 500 })
    }

    return NextResponse.json({ success: true })
  } catch (error: any) {
    console.error('SESSION COMMENT DELETE ERROR:', error)
