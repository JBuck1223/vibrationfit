...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!samples.length) {
      return NextResponse.json({ error: 'At least one text sample is required' }, { status: 400 })
    }
    ...
    if (upsertError) {
      ...
      return NextResponse.json({ error: 'Failed to save refined voice profile' }, { status: 500 })
    }

    return NextResponse.json({ voice_profile: upserted })
  } catch (error) {
    console.error('voice_profile analyze error', error)
    return NextResponse.json({ error: 'Failed to analyze voice profile' }, { status: 500 })
  }
}
