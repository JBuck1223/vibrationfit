...
export default function VisionBoardPage() {
  ...
  /** Open the full manifestation record. The lightbox stays only where the record page doesn't apply. */
  const openItem = (item: any) => {
    const isOthersItem = Boolean(item.member && !item.member.isSelf)
    if (isSnapshotMode || isIntensivePath || isOthersItem) {
      openLightbox(item.originalIndex)
      return
    }
    router.push(`/manifestations/${item.id}`)
  }

  /** Edit on the manifestation record page; in-place editor only where that page doesn't apply. */
  const editItem = (item: any) => {
    if (isSnapshotMode) return
    const isOthersItem = Boolean(item.member && !item.member.isSelf)
    if (isIntensivePath || isOthersItem) {
      startEditing(item)
      return
    }
    router.push(`/manifestations/${item.id}?edit=1`)
  }
    ...
    return (
      <div className="space-y-4 pt-3">
        <div className="space-y-2 pb-1 border-b border-neutral-700/50">
          ...
          >
            <Link href={`/manifestations/${item.id}?edit=1`}>
      ...
      <Stack gap="lg">
        ...
        <div className="flex items-center justify-between">
          <div className="flex-1 flex justify-start">
            {!isSnapshotMode ? (
              <button
                onClick={() => {
                  const base = isIntensivePath ? '/intensive/vision-board/new' : '/manifestations/new'
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              onClick={() => {
                ...
                const basePath = isIntensivePath ? '/intensive/vision-board/export' : '/manifestations/export'
        ...
        {loading || (isSnapshotMode && snapshotLoading) ? (
                      ...
                      if (boardMode === 'clean') {
                        ...
                        return (
                          <div
                            ...
                            onClick={() => bulkMode ? toggleItemSelection(item.id) : openItem(item)}
                          >
                            {bulkMode && (
                  ...
                  return (
                    <Card key={item.id} className={`transition-all duration-200 ${
                      ...
                      <div className="flex flex-col md:flex-row gap-3 md:gap-4">
                        ...
                        {/* Bulk checkbox + Image */}
                        <div
                          className="relative flex-shrink-0 w-full md:w-40 aspect-[4/3] rounded-lg overflow-hidden bg-neutral-800 cursor-pointer"
                          onClick={() => bulkMode ? toggleItemSelection(item.id) : openItem(item)}
                        >
                          {bulkMode && (
          ...
          <div className="text-center py-16">
            ...
            {items.length === 0 ? (
              <>
                <h3 className="text-xl font-semibold text-white mb-2">Your manifestation board is empty</h3>
                <p className="text-neutral-400 mb-6 max-w-md mx-auto">
                  Add your first creation — an image, goal, or moment you&apos;re calling in — to start
                  visualizing the life you choose.
                </p>
                <Button asChild>
                  <Link href={isIntensivePath ? '/intensive/vision-board/new' : '/manifestations/new'}>
