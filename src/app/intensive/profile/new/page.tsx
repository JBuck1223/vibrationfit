...
export default function IntensiveProfileNewPage() {
  ...
  const handleStartProfile = async () => {
    ...
    try {
      ...
      if (data.profile?.id) {
        router.push(`/intensive/profile/${data.profile.id}/edit`)
      } else {
        ...
        <div className="flex flex-col gap-2 md:gap-4 justify-center items-center max-w-2xl mx-auto">
          {profileStatus === 'completed' ? (
            <Button 
              variant="primary" 
              size="sm" 
              onClick={() => router.push(activeProfileId ? `/intensive/profile/${activeProfileId}` : '/intensive/profile')}
            ...
            <Button 
              variant="primary" 
              size="sm" 
              onClick={() => router.push(`/intensive/profile/${activeProfileId}/edit`)}
              className="w-full md:w-auto"
            >
              Continue Profile
