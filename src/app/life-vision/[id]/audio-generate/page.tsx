...
import { Headphones, CheckCircle, Play, CalendarDays, Moon, Zap, Sparkles, Plus, Music, Eye, X, AlertCircle } from 'lucide-react'
...
export default function AudioGeneratePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function loadData() {
    ...
    try {
      ...
      const preMadeVoices = (data.voices || []).map((v: any) => ({ 
        id: v.id, 
      ...
      const clonedVoices = (clones || []).map((clone: any) => ({
        id: `clone-${clone.elevenlabs_voice_id}`,
        name: clone.voice_name,
        isCloned: true,
        elevenLabsVoiceId: clone.elevenlabs_voice_id,
        previewUrl: clone.sample_audio_url
      }))
      
      setVoices([...clonedVoices, ...preMadeVoices])
    } catch {}
  ...
  async function handlePreviewGeneration() {
    ...
    try {
      const response = await fetch('/api/audio/preview-generation', {
        method: 'POST',
  ...
  async function handleTestVoice() {
    ...
    try {
      const response = await fetch('/api/audio/test-voice', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      audio.play().catch(err => console.error('Failed to play test audio:', err))
    } catch (error) {
  ...
  async function handleGenerateVoiceOnly() {
    ...
    try {
      ...
      console.log('🎤 [FRONTEND] Voice is cloned:', selectedVoiceForNew.startsWith('clone-'))
            ...
            <Card variant="glass" className="p-4">
              ...
              {testAudioUrl && (
                <div className="mb-4 p-4 bg-green-500/10 border border-green-500/30 rounded-xl">
                  <p className="text-sm text-green-400 mb-2">✅ Voice test successful! Listen to confirm it sounds right:</p>
              ...
              <div className="flex flex-col md:flex-row gap-4">
                ...
                <Button 
                  ...
                  disabled={testingVoice || !selectedVoiceForNew}
                  className="flex items-center gap-2"
                >
                  {testingVoice ? (
                    <>
          ...
          <div className="flex min-h-full items-center justify-center p-4">
            <Card className="max-w-2xl w-full my-auto">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-2xl font-bold text-white">🔍 Dry Run Complete!</h3>
                <button
              ...
              <div className="text-center mb-6">
                ...
                <p className="text-lg text-neutral-300 mb-2">
                  The system ran through the <strong>exact same process</strong> as a real generation, but without making any API calls.
                </p>
                <p className="text-sm text-neutral-400">
                  {previewData.sectionCount} sections processed in {(previewData.durationMs / 1000).toFixed(1)}s
                </p>
              </div>
              ...
              <div className="p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg mb-6">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-blue-400 font-medium mb-2">Check Your Terminal Logs</p>
                    <p className="text-blue-300 text-sm mb-3">
                      Look for lines starting with <code className="bg-black/30 px-2 py-0.5 rounded">[DRY RUN] 🔍</code> to see:
                    </p>
                    <ul className="text-blue-300 text-sm space-y-1 ml-4 list-disc">
                      <li>Exact character count per section</li>
                      <li>How text was processed for the variant</li>
                      <li>Number of API chunks per section</li>
                      <li>Total characters that would be sent</li>
              ...
              <div className="p-4 bg-green-500/10 border border-green-500/30 rounded-lg mb-6">
                <p className="text-green-400 font-medium mb-1">✅ This Matches Reality</p>
                <p className="text-green-300 text-sm">
                  The dry run uses the <strong>same exact code</strong> as real generation. Whatever character counts you see in the logs is what will actually be sent to ElevenLabs.
                </p>
              </div>
