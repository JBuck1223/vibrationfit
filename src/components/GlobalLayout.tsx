...
import { SidebarLayout, UserSidebar } from '@/components/SidebarClient'
...
export function GlobalLayout({ children }: GlobalLayoutProps) {
  ...
  return (
    <div className={cn(
      'bg-black text-white',
      isActivationExperience ? 'flex h-dvh flex-col overflow-hidden' : 'min-h-screen',
    )}>
      {!hideHeaderFooter && <Header />}
      <PageLayout className={cn(pageLayoutClass, audioPageLayoutClass, fullBleedClass)}>
        {children}
        {!isActivationExperience && <PlayerSpacer />}
      </PageLayout>
      {!hideHeaderFooter && !isActivationExperience && <Footer />}
    </div>
  )
}
