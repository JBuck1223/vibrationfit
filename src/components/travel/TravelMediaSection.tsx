...
import type { TravelAttachment } from '@/lib/travel/types'
import { isImageAttachment, isVideoAttachment, uploadTravelFiles } from './travel-utils'

interface TravelMediaSectionProps {
  /** Attachment API base, e.g. /api/travel/trips/{id} or /api/travel/dream-destinations/{id} */
  endpoint: string
  attachments: TravelAttachment[]
  onChanged: () => void
}

export function TravelMediaSection({ endpoint, attachments, onChanged }: TravelMediaSectionProps) {
  const [uploading, setUploading] = useState(false)
  const [uploadPercent, setUploadPercent] = useState(0)
  const [lightboxIndex, setLightboxIndex] = useState<number | null>(null)
  const [playingVideo, setPlayingVideo] = useState<TravelAttachment | null>(null)
  const [deleting, setDeleting] = useState<TravelAttachment | null>(null)
  ...
  const handleFiles = async (selected: File[]) => {
    ...
    setUploading(true)
    try {
      const uploaded = await uploadTravelFiles(selected, setUploadPercent)
  ...
  const deleteAttachment = async () => {
    ...
    setIsDeleting(true)
    try {
      const res = await fetch(`${endpoint}/attachments?attachment_id=${deleting.id}`, {
        method: 'DELETE',
      })
      if (!res.ok) throw new Error()
      setDeleting(null)
  ...
  const mediaTiles: { attachment: TravelAttachment; kind: 'image' | 'video'; imageIndex?: number }[] = [
  ...
  return (
    <div>
      ...
      <DeleteConfirmationDialog
        ...
        title="Delete Media"
        message="This removes the file from this record."
