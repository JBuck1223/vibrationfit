...
import { DailyPaperEditForm } from './DailyPaperEditForm'
import type { DailyPaperEntry } from '@/hooks/useDailyPaper'

export interface DailyPaperEditModalProps {
  isOpen: boolean
  entry: DailyPaperEntry | null
  onClose: () => void
  onSuccess: () => void
}

export function DailyPaperEditModal({
  isOpen,
  entry,
  onClose,
  onSuccess,
}: DailyPaperEditModalProps) {
  useEffect(() => {
    if (!isOpen) return
