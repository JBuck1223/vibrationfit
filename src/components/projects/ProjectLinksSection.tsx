...
import type { ProjectReferenceLink } from '@/lib/projects/types'
import { faviconUrl, linkHost } from './media-utils'
...
export function ProjectLinksSection({ projectId, taskId, links, onChanged }: ProjectLinksSectionProps) {
  ...
  const addLink = async () => {
    ...
    try {
      const res = await fetch(`/api/projects/${projectId}/links`, {
        ...
        body: JSON.stringify({ url, title, ...(taskId ? { task_id: taskId } : {}) }),
      })
  ...
  const deleteLink = async (linkId: string) => {
    const res = await fetch(`/api/projects/${projectId}/links?link_id=${linkId}`, {
      method: 'DELETE',
    })
    if (res.ok) {
      onChanged()
    ...
    <div>
      {links.length === 0 ? (
        <p className="py-4 text-center text-sm text-neutral-500">
          ...
          No links yet. Save references, docs, and inspiration here.
        </p>
      ) : (
