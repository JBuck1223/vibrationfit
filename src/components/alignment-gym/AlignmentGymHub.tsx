...
export interface AlignmentGymHubProps {
  /** Highlights a section during intensive walkthrough */
  activeTourAnchor?: AlignmentGymTourAnchor | null
  /** Open "What's the Alignment Gym" during walkthrough */
...
const TOUR_RING =
  'ring-2 ring-[#00FFFF]/60 shadow-[0_0_24px_rgba(0,255,255,0.12)] rounded-2xl transition-shadow duration-300'
...
export function AlignmentGymHub({
  ...
  useEffect(() => {
    if (!activeTourAnchor) return
    const el = document.querySelector(`[data-alignment-gym-tour="${activeTourAnchor}"]`)
    el?.scrollIntoView({ behavior: 'smooth', block: 'center' })
  }, [activeTourAnchor])

  const tourClass = (anchor: AlignmentGymTourAnchor) =>
    cn(activeTourAnchor === anchor && TOUR_RING)
    ...
    <Container size="xl">
      <Stack gap="lg">
        ...
        <div
          ...
          className={cn(
            'relative rounded-2xl border border-white/[0.06] bg-gradient-to-br from-[#14B8A6]/[0.04] via-[#111] to-[#111]',
            tourClass('stats'),
          )}
        >
        ...
        <div
          data-alignment-gym-tour="what-is"
          className={cn(
            'rounded-2xl border-2 border-[#333] bg-[#1F1F1F] overflow-hidden transition-all duration-300 hover:border-[#444]',
            tourClass('what-is'),
          )}
        >
          <button
