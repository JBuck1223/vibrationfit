...
import { TrackArtistLine, TrackNowPlayingMeta, MusicTrackListSubtitle, isSongNowPlayingTrack } from './TrackArtistLine'
...
export function EmbeddedPlayer({
  ...
  const openTrackMenuAt = useCallback((trackId: string, anchor: { top: number; right: number; openUp?: boolean }) => {
    setOpenMenuTrackId(trackId)
    setMenuPosition(anchor)
  }, [])

  const openTrackMenuFromButton = useCallback((trackId: string, button: HTMLButtonElement) => {
    const rect = button.getBoundingClientRect()
    const menuHeightEstimate = 220
    const spaceBelow = window.innerHeight - rect.bottom
    const openUp = spaceBelow < menuHeightEstimate && rect.top > menuHeightEstimate
    openTrackMenuAt(trackId, {
  ...
  const startLongPressMenu = useCallback((trackId: string) => {
    longPressTimerRef.current = window.setTimeout(() => {
      const btn = menuButtonRefs.current.get(trackId)
      if (btn) openTrackMenuFromButton(trackId, btn)
      longPressTimerRef.current = null
    }, 500)
  }, [openTrackMenuFromButton])

  const cancelLongPressMenu = useCallback(() => {
  ...
  }, [openMenuTrackId, closeTrackMenu])
    ...
    <div className={cn(`${panelBg} border border-neutral-800 rounded-2xl overflow-hidden`, className)}>
      ...
      <div className={cn(panelBg, 'flex flex-col items-center px-6 pt-6 pb-4')}>
          ...
          if (enableArtworkLightbox && displayTrack?.thumbnail) {
            return (
              <button
                ...
                className="group relative mb-6 flex-shrink-0 rounded-2xl focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/50"
                aria-label="View album art full size"
              >
                <TrackArtwork
                  track={displayTrack}
                  iconKey={setIconKey}
                ...
                />
                <span className="pointer-events-none absolute inset-0 flex items-center justify-center rounded-2xl bg-black/0 opacity-0 transition-opacity group-hover:bg-black/30 group-hover:opacity-100">
        ...
        <div className="text-center mb-5 w-full">
          ...
          {isSongNowPlayingTrack(activeTrack, contentCategory, mapActivityType) ? (
            <div className="mt-1 w-full">
              <TrackNowPlayingMeta track={activeTrack} />
            </div>
          ) : contentCategory !== 'story' ? (
            <p className="text-sm text-neutral-400 mt-1 truncate text-center">
          ...
          {tracks.map((track, index) => {
            ...
            return (
              <div
                key={track.id}
                className={cn(
                  'relative flex items-center gap-0.5 rounded-lg pr-1',
                  isActive && 'bg-primary-500/15'
                )}
                onContextMenu={(e) => {
                  e.preventDefault()
                  openTrackMenuAt(track.id, {
                    top: e.clientY,
                    right: window.innerWidth - e.clientX,
                  })
                }}
                onTouchStart={() => startLongPressMenu(track.id)}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 min-w-0">
                      {track.versionLabel ? (
                        <span className="truncate text-xs text-neutral-500">{track.versionLabel}</span>
                      ) : isSongNowPlayingTrack(track, contentCategory, mapActivityType) ? (
                      ...
                      {track.publishStatus === 'published' && (
                        <span className="flex-shrink-0 px-1.5 py-0.5 rounded text-[10px] font-medium leading-none bg-[#39FF14]/15 text-[#39FF14]">Published</span>
                      )}
                      {track.publishStatus === 'approved' && (
                        <span className="flex-shrink-0 px-1.5 py-0.5 rounded text-[10px] font-medium leading-none bg-[#BF00FF]/15 text-[#BF00FF]">Approved</span>
                      )}
                      {track.publishStatus === 'pending' && (
                        <span className="flex-shrink-0 px-1.5 py-0.5 rounded text-[10px] font-medium leading-none bg-[#00FFFF]/15 text-[#00FFFF]">Submitted</span>
                      )}
                      {track.memberCreated && !track.publishStatus && (
                        <span className="flex-shrink-0 px-1.5 py-0.5 rounded text-[10px] font-medium leading-none bg-[#FFFF00]/10 text-[#FFFF00]">Member Created</span>
                      )}
                    </div>
                  </div>
                ...
                <div className="relative flex-shrink-0">
                  <button
                    ...
                    ref={(el) => {
                      ...
                      else menuButtonRefs.current.delete(track.id)
                    }}
                    onClick={(e) => {
                      e.stopPropagation()
                      if (isMenuOpen) closeTrackMenu()
                      else openTrackMenuFromButton(track.id, e.currentTarget)
                    }}
                    className={cn(
                      'p-1.5 rounded-lg transition-colors text-neutral-500 hover:text-neutral-300',
                      isMenuOpen && 'text-neutral-200 bg-neutral-800/80'
                    )}
                  >
      ...
      {openMenuTrackId && menuPosition && typeof document !== 'undefined' && (() => {
        const menuTrack = tracks.find(t => t.id === openMenuTrackId)
        const menuIndex = menuTrack ? tracks.findIndex(t => t.id === openMenuTrackId) : -1
        if (!menuTrack || menuIndex < 0) return null

        const menuCached = cachedTrackIds.has(menuTrack.id)
        const menuDownloading = downloadingTrackIds.has(menuTrack.id)
        const menuSaving = savingTrackIds.has(menuTrack.id)

        return createPortal(
          <>
            <div className="fixed inset-0 z-[200]" onClick={closeTrackMenu} />
            <div
              role="menu"
              className="fixed z-[201] min-w-[190px] rounded-xl border border-neutral-700/80 bg-neutral-900 py-1 shadow-2xl"
              style={{
            ...
            >
              {onAddToPlaylist && (
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation()
                    closeTrackMenu()
                    onAddToPlaylist(menuTrack, menuIndex)
                  }}
              ...
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation()
                  closeTrackMenu()
                  menuCached ? removeTrack(menuTrack.id) : downloadTrack(menuTrack)
                }}
                className={cn(
                  'flex w-full items-center gap-2.5 px-3 py-2 text-sm text-neutral-200 transition-colors hover:bg-neutral-800',
                  menuDownloading && 'cursor-wait opacity-60'
                )}
              >
                {menuDownloading ? (
                ...
                {menuCached ? 'Remove offline copy' : menuDownloading ? 'Caching...' : 'Play offline'}
              </button>
              <button
                type="button"
                role="menuitem"
                ...
                onClick={(e) => {
                  e.stopPropagation()
                  closeTrackMenu()
                  saveTrackToDevice(menuTrack)
                }}
                className={cn(
                  'flex w-full items-center gap-2.5 px-3 py-2 text-sm text-neutral-200 transition-colors hover:bg-neutral-800',
                  menuSaving && 'cursor-wait opacity-60'
                )}
              >
                {menuSaving ? (
                  <Loader2 className="w-4 h-4 animate-spin text-neutral-400" />
                ...
                <>
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation()
                      closeTrackMenu()
                      onRemoveTrack(menuTrack, menuIndex)
                    }}
                    className="flex w-full items-center gap-2.5 px-3 py-2 text-sm text-[#FF0040] transition-colors hover:bg-[#FF0040]/10"
                  >
      ...
      {enableArtworkLightbox && (activeTrack || tracks[0])?.thumbnail && (
        <ImageLightbox
          images={[{
            url: (activeTrack || tracks[0])!.thumbnail!,
            alt: (activeTrack || tracks[0])?.title || 'Album art',
          }]}
          currentIndex={0}
