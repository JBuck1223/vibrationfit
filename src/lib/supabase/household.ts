...
export async function invitePartnerToHousehold(params: {
  ...
  try {
    ...
    // Ensure partner has a Supabase auth account.
    // Try to create - will silently fail if they already exist.
    ...
    // Always generate a magic link so the partner lands on
    // setup-password (if needed) → invite page.
